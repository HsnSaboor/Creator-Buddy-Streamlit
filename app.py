...
if __name__ == "__main__":
    ...
    if st.button("Analyze Video"):
        with st.spinner("Extracting data..."):
            ...
            st.write(f"Extracting data for video URL @ start_time: {datetime.datetime.now()}")

            try:
                asyncio.run(extract_video_data(video_url))
                # Load the extracted data from the saved Markdown file
                end_time = datetime.datetime.now()
                st.write(f"Finished extracting data for video URL @ end_time: {datetime.datetime.now()}")

                video_id = parse_qs(urlparse(normalize_youtube_url(video_url)).query)['v'][0]
